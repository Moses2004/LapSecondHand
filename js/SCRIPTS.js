...
function adjustUIForLoginStatus() {
    ...
    if (!authBox || !nav) return;

    
    const existingOrdersLink = nav.querySelector('.orders-link');
    ...
    if (isUserLoggedIn) {
        ...
        // Add  link dynamically if user is logged in
        const contactLink = nav.querySelector('a[data-target="contact"]');
        const ordersLink = document.createElement('a');
        ordersLink.href = '../php/view_order.php';
        ordersLink.className = 'nav-link orders-link';
        ordersLink.textContent = 'Your Orders';
        if (contactLink) {
            nav.insertBefore(ordersLink, contactLink.nextSibling);
        } else {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchPhoneDetails(phoneId) {
        ...
        try {
            ...
            if (data.success && data.data) {
                ...
                let buyButtonHTML = '';
                if (isUserLoggedIn) {
                    buyButtonHTML = `<button id="buyNowBtn" style="padding: 10px 20px; background-color: #003366; color: white; border: none; border-radius: 5px; cursor: pointer;">Buy Now</button>`;
                } else {
                ...
                if (buyNowBtn) {
                    buyNowBtn.addEventListener('click', () => {
                        window.location.href = `../php/order.php?phone_id=${phoneId}`;
                    });
                }
            } else {
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (event) => {
            // Only handle internal links with a hash (#)
    ...
    const handleHashChange = () => {
        ...
        // Deactivate all nav links except the "Your Orders" link
        const coreNavLinks = document.querySelectorAll('nav .nav-link:not(.orders-link)');
        coreNavLinks.forEach(link => link.classList.remove('active'));
        ...
        if (detailMatch) {
            ...
            const targetLink = document.querySelector(`nav a[data-target="${newHash}"]`);
    ...
    // Initial call to handleHashChange on page load
    handleHashChange();
